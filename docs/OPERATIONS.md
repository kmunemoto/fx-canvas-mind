...
**注文にはならない。** #37 の実測: AI に約定価格を選ばせていた時期、売買 8 件の
うち 5 件が未約定で、その 5 件すべてに AI 自身の Trend Day / Breakout タグが
シグナルと同じ向きに付いていた。`should_be_market` はその形を拒むために書かれた
...
- **トレードの成績と並べてはいけない。** 損切りも利確も最大逆行も無いので、
  +0.217 R/件 と同じ軸には乗らない。最長の期限では「当たり」判定の過半が、
  このアプリ自身の最小損切り幅で先に切られていた主張である。
...
| 関数 | 版 | bytes | 変わった理由 |
|---|---|---|---|
| analyze | v57 → **v58** | 102,671 | 本体。宣言・保存・プロンプト |
| postmortem | v28 → **v29** | 96,082 | `econ-calendar/events.ts` 経由で `_shared/horizon.ts` を取り込む（108 B）。**挙動は不変** |
| version-compare | v9 → **v10** | 85,050 | 同上。**挙動は不変** |
| position-review | v1 → **v2** | 27,026 | `analyze/locale.ts` を取り込むため。**サイズもトークン数も HEAD と同一**（27,026 / 3,654）で、差は minifier の識別子名だけ |
...
### 照合が 1 文字の取り違えを捕まえた（analyze、1 回目）

analyze の 1 回目のデプロイは**バイト一致しなかった**。差は 1 文字だけで、
`conditional-wait.ts` の `(regime === "Up") === (then === "BUY")` が本番側では
`===`、ローカルでは `==` になっていた。**どちらも真偽値どうしの比較なので意味は同じ**
だが、意味が同じだから良いという話ではない: バンドルを丸ごと書き出す経路は
1 文字を取り違えうる、という #48 / #51 と同じ失敗が**もう一度起きた**ということである。
2 回目で `e14952c4afc49744…` が一致し、fn version は 78 になった。

**この照合が無ければ、版名だけは v58 なのに中身が違うものが本番に乗っていた。**
両方のファイルが同じ版名を持っていたので、**版名では区別できなかった**。
