...
- **メール送信は Resend**。関数のシークレット `RESEND_API_KEY` が無い間は、サインを検出して `not_configured` で記録するだけで**メールは出ない**（画面にもそう出す）。
  - 送信元は既定で `Sextant <onboarding@resend.dev>`。この送信元は Resend のアカウントを作ったメールアドレス宛てにしか届かない。他の人（お客様）に送るには Resend で独自ドメイン（fx-tactical.jp など）を認証し、`ALERT_FROM`（例 `Sextant <alerts@fx-tactical.jp>`）を設定する。
