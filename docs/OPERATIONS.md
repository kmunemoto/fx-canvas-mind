...
| EFFORT_TECHNICAL | `medium` | `max` → **`medium` に差し戻し** |
| EFFORT_SEARCH | `low` | `max` → **`low` に差し戻し** |
| max_tokens | 8000 | 16000 |

effort の範囲は `low` < `medium` < `high` < `xhigh` < `max`。

> **`max` は約100分だけ本番に乗り、差し戻した。** 下の「8.5-a」を読むこと。
> model と max_tokens は維持している。壊れたのは effort 1点だった。
...
effort 2値だけ。**model（Sonnet 5）と max_tokens（16000）は維持**した。
壊れたのは effort 1点なので、そこだけ戻す。

`noise-floor/shape.ts` の固定ミラーも同時に戻した。結果として
`PRE_SWITCH_EFFORT_*` と現行値は再び等しくなっているが、これはバグではない——
**`max` で書かれた行は1行も存在しない**（試みた2ターンはどちらも壁時計で死んだ）ので、
effort についてフォールバックが2つの時代を区別する必要がない。
`max_tokens` は異なったままで、そちらが恒久的に動いた値である。
