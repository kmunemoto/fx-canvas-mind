...
- **過去の週末の足は系列の途中に残っている（未対処）**: 下見の切り詰めは末尾だけなので、1 つ前の週末の平坦な足は 1h の 250 本の中に残る（10.4 日 ≒ 1.5 週末 ≒ 70 本、約 2 割）。
  これは下見に限らず **平日の分析にもずっと入っていた**。ATR14 のような短い窓では古い週末の影響は減衰するが、SMA200 や 200 本規模の窓には効く。
  直すなら analyze 側でも `isMarketClosed` で常時落とし、落とした分だけ取得本数を増やす必要がある（1h で SMA200 に 200 本の**取引時間**が要るので、暦の 250 本では足りない）。範囲が平日の全分析に及ぶので、下見の修正とは分けてある。
