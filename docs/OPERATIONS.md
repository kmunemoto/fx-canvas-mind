...
エントリー足の 1 つ下の足（`LOWER_TIMEFRAME`: 15min→5min, 1h→15min, 4h→1h,
1day→4h）を 48 本、**GMO Coin の仲値**で取る。Twelve Data の 8 リクエスト/分の
枠を使わないのが選定理由。
...
ATR で 0.25 未満（`too_close`）/ 3.0 超（`too_far`）、ATR が無い（`no_atr`）、
欠損（`malformed`）、WAIT でない回（`not_a_wait`）を名前付きで捨て、理由を
`entry_check.conditional_rejection` に残す。**黙って捨てない**: 出力の 8 割が
