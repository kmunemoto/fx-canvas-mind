...
- **層別 1（深さ）**: `prior.bars_after_settlement <= 8` の群と `> 8` の群で `cause` 変化率を比べる
- **層別 2（語彙）**: `prior.version` が **v16 以降**（＝いまと同じ原因語彙）の群と、v1 / v3 / v7 の群を分ける。§4.1 を参照
...
### 4.1 語彙の交絡（2026-09-08 03:15 追記）

**この節はデプロイ後、最初の 3 行が入った時点で追記した。** 追記の理由は 3 行の結果ではなく、
**3 行とも前の読みが v1（9/3）だった** という母集団の性質に気づいたことである。透明性のため、その 3 行の結果もここに書く:
`46c7c080`（1day、21→119 本）cause 不変・`avoidable` true→false、`2eb99b9d`（1h、13→95 本）entry_too_far→**inconclusive**、
`7b059b5f`（1h、13→95 本）変化なし。

問題はこうである。**v21 は診断の入力を変えていないが、`prior` の側は同じ版で書かれていない。**
読み直し対象の残り（トレード行）を前の読みの版で数えると:

| 前の読みの版 | トレード行 |
|---|---|
| v1 / v3 / v7（旧語彙。`entry_too_far` などが現役だった） | 9 |
| v16 以降（現行語彙） | 10 |

旧語彙の行で「`cause` が変わった」は、深さのせいか語彙が変わったせいか分離できない。
`entry_too_far` は現行契約では出せない原因なので（`LEGACY_CAUSES`）、**旧語彙の行はほぼ必ず変わる**。
主指標を全件で取ると、この機械的な変化が「深さの効果」として数えられる。§3 の予備調査で踏んだのと同じ穴である。

**したがって §6 の判定は v16 以降の群で行う。** 全件と旧語彙群は並べて報告するが、結論は出さない。
v16 以降の群は 10 件前後しかなく、20% は 2 件を意味する。**弱い。それを先に書いておく。**
...
- **語彙条件**（§4.1）: 全件では 20% を超えるのに、**v16 以降の群では 20% 未満**。
  → 変化しているのは語彙であって深さではない。この場合「効いた」とは書かない。
  逆に v16 以降の群だけで 20% を超えたなら、n が 10 件前後しかないことを添えて報告する。
