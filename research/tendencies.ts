...
import { fetchPair as fetchGmo } from "./gmo.ts";
...
// Fetching, caching and the SYNTHETIC random walk live in gmo.ts, shared with
// indicators.ts (#102) so both studies read the same bars.
const fetchPair = (pair: string) => fetchGmo(pair, { start: START, now: NOW, cache: CACHE, concurrency: CONCURRENCY });
