...
// stochastic's lengths — one choice for every chart, kept in this browser
...
export const CHART_PREFS_DEFAULTS: ChartPrefs = { rsi: true, stoch: true, stochParams: STOCH_DEFAULTS };
...
export const setChartPrefs = (patch: Partial<ChartPrefs>): void => {
  ...
  current = { ...next, stochParams: normalizeStochParams(next.stochParams) };
  try {
