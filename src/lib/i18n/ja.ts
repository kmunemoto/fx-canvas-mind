...
export const ja = {
  ...
  control: {
    ...
    cost: (tf: string, share: number | null) =>
      share === null
        ? tf === "1min"
          ? "1分足はスプレッドの負担が最も大きくなります（割合は未測定）。"
          : "日足はスプレッドの負担が最も小さい側です（数字は未測定）。"
        : `${tf}はスプレッドだけで損切り幅の約${Math.round(share * 100)}%を毎回失います（過去の測定）。${share >= 0.1 ? "4時間足・日足の方が負担は小さくなります。" : ""}`,
    analyze: "分析開始",
    analyzing: "分析中...",
    stages: {
      idle: "",
  ...
  result: {
    ...
    // 15分/1時間/4時間/日足で 1.05 / 1.67 / 1.26 / 2.82 本 — つまり本数で
    // 見るとほぼ時間足によらない。だから期間は本数で宣言する。
    horizon: {
      label: "狙う期間",
      bars: (tfLabel: string, bars: number, span: string) =>
        `${tfLabel}で${bars}本（市場が開いている時間で${span}）`,
      // 日足で「約120時間」と書いても読めないので、長い期間は日で言う。
      // 数の丸めは辞書側に置く（画面側で足すと言語ごとにずれる）。
      span: {
        minutes: (n: number) => `約${n}分`,
        hours: (n: number) => `約${n}時間`,
        days: (n: number) => `約${n}日`,
      },
      // 市場時間で歩いた終端。30分刻みで求めているので「おおよそ」と書く。
      endsAt: (when: string) => `おおよそ ${when} まで`,
      // ここが一番誤解されやすい。実測: 決着した53件を24本で打ち切ると
      // 2件が落ちて、その2件は**どちらも勝ち**だった。期限ではない。
      notACutoff: "本数は期限ではありません。決着するまで採点は続きます。",
      // TP1 と TP2/TP3 の役割の違い。期間はプランの水準の置き方を変える。
      tpRoles: "TP1 はこの期間内に届くことを狙う水準です。TP2・TP3 はその先の伸びで、期間内に届く前提ではありません。",
      // 期間の記録が無い行。117件の既存行は意図的に遡って埋めていない。
      absent: "この分析には狙う期間が記録されていません。",
      // 1分足（#98）だけに出す。価格を読んでから画面に出るまで実測で中央値
      // 約50秒かかり、1分足ではその間にほぼ1本進む。エントリー価格は「その
      // 時点の現在値」であって、表示された瞬間に約定できる価格ではない。
      staleness: (time: string, seconds: number) =>
        `価格を読んだのは ${time}（表示時点で約${seconds}秒前）。1分足ではその間に相場が動いています。エントリー価格と今の価格の差を確かめてから判断してください。`,
    },
    ...
    counterCase: {
      ...
      trigger: "乗り換える条件",
      note: "AI が判断と逆の方向について書いた最強のケースです（v68 以降は任意で、売買の判定には使っていません）。",
    },
    detail: "詳細分析",
  ...
  chart: {
    ...
    zoneShiftNote: (bars: number | null, status: "loading" | "ready" | "error") =>
      ...
      "過去のチャート（11ペア・15分/1時間/4時間、損切り ATR×1・利確2倍、スプレッド込み、2025-07以降）: 転換で入ると 5,687回・勝率29.7%（損益ゼロは33.3%）・1回あたり −0.109R、◆で入ると 9,028回・30.4%・−0.085R。どちらもランダムに入った場合との差は誤差の範囲（1時間足だけ少し良いが、それでもほぼ0）。" +
    ...
    fvgProfileNote: (bars: number) =>
      ...
      "【箱】未充填のフェアバリューギャップ（3本の足の間の空白）の上に逆向きの新しいギャップが重なった帯だけを表示。緑=買い側・赤=売り側（新しいギャップの向き）。逆向きのギャップがまた重なると反転し、重なった部分だけに狭まる。★=形成と反転の回数、▲▼=帯から離れた後に戻ってきた足（再テスト）、薄い斜めの帯=元のギャップ、薄い箱=終値が反対側を抜けて終わった帯。確定足だけで判定。チャートに出ている足（リアルタイムは120本）の中だけで探すので、元の3,000本より帯は少なめ。" +
      `【左の横棒】直近${bars}本${bars < 200 ? "（チャートにある全部。元は200本）" : ""}の値幅を30段に分け、各段を通った足の数。緑=陽線・赤=陰線、長いほど多い。黄色の線=いちばん多い段（POC）。元は足ごとの出来高で数えますが、GMO の FX の足には出来高がないため、1本=1として数えています（価格がそこに留まった時間の目安で、出来高ではありません）。ローソク足の下に半透明で描画（元は足の上に不透明な箱）。` +
      "表示のみで、サインの判定・メールには使っていません。",
    kalmanNote:
      "SPECTRA型 = SentioEdge の SPECTRA の公開説明の処理順を再現したもの：高値と安値の中間と ATR(10) をカルマンフィルターで平滑 → スーパートレンド（ATR×3）→ RSI(14) が50の上（売りは下）のときだけ ▲▼。緑=上昇・赤=下降、雲=平滑した価格との間。Smart Trail（公開されていない）と出来高の分類（GMO の足に出来高が無い）は入っていません。確定足だけで印を付けます。過去の検証（11ペア・15分/1時間/4時間、損切り ATR×1・利確2倍、スプレッド込み、2025-07〜）: 7,164回・勝率29.4%（損益ゼロは33.3%）・1回あたり −0.116R で、ランダムに入った場合と差がありませんでした（1時間足は −0.031R・差 +0.05R だが誤差の範囲）。RSI のフィルターが外したのは約7,000回中1回だけでした。サインの判定・メールには使っていません。",
    hide: "隠す",
    show: "表示する",
    foldList: "一覧をたたむ",
    tabsLabel: "時間足",
  ...
  history: {
    ...
    gate: {
      ...
      reasons: {
        ...
        // 損切りの下限（ATR倍）と同じ要求を、エントリーの反対側にもする。
        // 2026-09-21 に損切り・利確とも ATR×0.6 を下限にした。
        target_too_close: "利確1が近すぎる（届いても読みが当たった証拠にならない）",
    ...
    postmortem: {
      ...
      candidateArm: "この回は候補版の腕（variant）で出した判断です。原因分析は共有ルールブックの学習に流れるので対照版だけで回します。この行は分析されません。",
  ...
  rules: {
    ...
    showAll: (n: number) => `すべて表示（${n}件）`,
    showLess: "折りたたむ",
    note: "実際の値動きに基づく検証から自動生成されています。売買の判定は RSI とパラボリックSAR のルールで行うため、これらのルールは分析には使っていません（記録として表示しています）",
  ...
  scores: {
    ...
    // (d) 期間内決着（#91 step 2）。上の3つと違い、**どちらが良いとも言っていない**。
    // 伸びた勝ちは期間外に決着する。実測: 決着53件を24本で打ち切ると落ちる2件は
    // どちらも勝ちだった。だから色で塗らず、その旨を行に書く。
    pace: {
      label: "狙った期間の内に決着した割合",
      notAScore: "これは成績ではありません。大きいほど良い・小さいほど悪い、のどちらでもありません（伸びた勝ちは期間の外で決着します）。",
      hint: "分母は「決着していて、かつ狙う期間を宣言していた」取引だけです。上の3つとは母集団が違います（上の3つは診断が終わった行だけを見ます）。",
      noHorizon: (n: number) => `期間の記録が無い決着 ${n}件`,
      openPast: (n: number) => `期間を過ぎてまだ保有中 ${n}件`,
    },
    deepMae: {
  ...
  position: {
    ...
    registerErrors: {
      ...
      // #92（保有中の建玉を直接登録する経路）の拒否。プラン経由とは別の RPC なので、
      // 文言も別に持つ — プランの水準ではなく**利用者自身の水準**を断っている。
      pair_invalid: "通貨ペアの書式が不正です（例: USD/JPY）",
      interval_invalid: "対応していない時間足です（15分・1時間・4時間・日足のいずれか）",
      direction_invalid: "売り・買いのどちらかを選んでください",
      stop_loss_must_be_positive: "損切りは 0 より大きい数で入力してください",
      take_profit_1_must_be_positive: "利確 TP1 は 0 より大きい数で入力してください",
      levels_incoherent: "水準の向きが合っていません。買いなら 損切り < 約定 < TP1、売りなら TP1 < 約定 < 損切り です",
      targets_out_of_order: "利確の順番が合っていません（TP2 を飛ばして TP3 だけ、も受け付けません）",
      generic: "登録できませんでした。時間をおいて再試行してください",
    },
    openTitle: "保有中の建玉",
    // #92: アプリのプランから入ったのではない建玉を登録する。プラン経由の
    // エントリー登録とは別の入口・別の RPC で、水準は利用者自身のものを受け取る。
    own: {
      button: "持っている建玉を登録",
      title: "すでに持っている建玉",
      hint: "アプリのプランから入ったものでなくても登録できます。損切りと利確は**あなたが置いたもの**を入れてください。次回の分析から、この水準で「保有中の判断」が出ます。",
      notCounted: "この建玉はアプリの成績には入りません。アプリが出した判断ではないためです。",
      whyLevels: "損切りと利確が必須なのは、これが無いと含み R も損切り接触も計算できず、保有中の判断が何も言えないからです。",
      pair: "通貨ペア",
      interval: "見直す足",
      direction: "売買",
      buy: "買い",
      ...
      tp3: "利確 TP3（任意）",
      fillTime: "約定時刻（任意）",
      fillTimeHint: "未入力なら登録した時刻を記録します。古い約定でもかまいません",
      submit: "登録する",
      cancel: "やめる",
      registered: "保有中の建玉を登録しました",
    },
    openedAt: "建玉",
    openedAtRegistered: "登録時刻で記録",
    planTimeframe: "分析足",
    // 紐づくプランが無い建玉では「分析足」がプランの足を指さないので、言い方を分ける。
    ownTimeframe: "見直す足",
    // プランに紐づかない建玉（#92）向けの言い換え。水準も根拠も利用者のものなので、
    // 「元のプランの〜」と書くとその行が嘘になる。
    heldSubtitleOwn: "登録済みの建玉を、あなたが登録した水準で評価しています。元になったプランはありません。新規判断とは別です。",
    closeButton: "決済した",
    ...
    // プランに紐づかない建玉（#92）向け。上の文は「根拠」＝元のプランの根拠を
    // 指しており、プランが無い行では**維持される根拠が存在しない**。
    // 「元になったプランはありません」の 3 行下で「根拠は維持されており」と
    // 書くのは、同じカードの中での自己矛盾である。
    verdictGlossOwn: {
      hold: "あなたが登録した撤退条件は成立していません",
      caution: "不利な事実があります。あなたが登録した撤退条件は成立していません",
      exit_condition_met: "あなたが登録した撤退条件に達しています",
      undecidable: "材料が足りず、判定できません",
    },
    ...
    facts: {
      ...
      // 障害ではなく、そもそも決着させるプランが無い（#92）。
      trackerNoPlan: "紐づくプランなし",
  ...
  index: {
    ...
    upgradeBody: "分析回数の上限解放・優先サポートが利用できます",
    upgradeCta: "アップグレード",
  ...
  pricing: {
    ...
    features: {
      ...
      pro: ["無制限の分析", "全機能", "売買サインのメール通知", "優先サポート"],
    },
  },
  ...
  lp: {
    ...
    subtitleBefore: "判断に使う指標は",
    subtitleCount: "RSI とパラボリックSAR",
    subtitleAfter: "の2つだけ。確定足で条件がそろったときだけ売買サインを出し、そろっていないときは次の足がいくらで引ければ条件がそろうかを示します。さらに全プランを実際のBid/Askで自動採点し、外れた分は原因を調べて記録します。",
    ...
    features: [
      { title: "全自動データ取得", desc: "リアルタイム価格から RSI(14) とパラボリックSAR を計算し、今週の経済指標カレンダーも自動取得" },
      { title: "過去のサインをチャートに表示", desc: "RSI とパラボリックSAR の条件がそろった足に BUY・SELL を描き、そのときの損切り・利確と勝ち負けまで表示します" },
    ...
    loopSteps: [
      ...
      { title: "記録", desc: "特定した原因をルールブックとして記録し、何件の実績に裏付けられているかまでアプリ内で確認できます。売買の判定そのものは RSI とパラボリックSAR のルールで行います" },
    ],
    ...
    faqTitle: "よくある質問",
    faqs: [
      { q: "Sextantとは何ですか？", a: "為替のテクニカル分析ツールです。RSI(14) とパラボリックSAR の2つだけで売買の条件を判定し、1分足・15分足・1時間足・4時間足・日足で使えます。" },
      { q: "他のAI分析ツールと何が違いますか？", a: "出した予想を必ず採点し、外れた理由を自動で調べる仕組みが入っている点です。全プランは期限まで実際のBid/Askで追跡され、外れたものは決着後の値動きから原因を特定してルールブックとして記録されます。ルールの内容と、そのルールが何件の実績に裏付けられているかはアプリ内で確認できます。" },
      ...
      { q: "どのようなテクニカル指標を使用していますか？", a: "RSI(14) とパラボリックSAR(0.02, 0.2) の2つです。RSI が30または70から戻り、SAR が同じ側にある確定足で売買サインを出します。損切りと利確の幅には ATR(14) を使います。過去のチャートでの検証では、このルールの勝率は損益ゼロに届いていません。" },
