...
export const ja = {
  ...
  chart: {
    ...
    vpNote: (bars: number) =>
      `Weighted Volume Profile（Flux Charts の公開コード〈MPL 2.0〉を移植）: 直近${bars}本${bars < 200 ? "（チャートにある全部。元は200本）" : ""}の値幅を30段に分け、各段を通った足の数を横棒で表示。緑=陽線・赤=陰線、長いほど多い。黄色の線=いちばん多い段（POC）。元は足ごとの出来高で数えますが、GMO の FX の足には出来高がないため、ここでは1本=1として数えています（価格がそこに留まった時間の目安で、出来高ではありません）。ローソク足の下に半透明で描画（元は足の上に不透明な箱）。表示のみで、サインの判定・メールには使っていません。`,
    fvgNote:
      "FVG Crossfire（FluxChart の公開コード〈MPL 2.0〉をそのまま移植）: 未充填のフェアバリューギャップ（3本の足の間の空白）の上に逆向きの新しいギャップが重なった帯だけを箱で表示。緑=買い側・赤=売り側（新しいギャップの向き）。逆向きのギャップがまた重なると反転し、重なった部分だけに狭まる。★=形成と反転の回数、▲▼=帯から離れた後に戻ってきた足（再テスト）、薄い斜めの帯=元のギャップ、薄い箱=終値が反対側を抜けて終わった帯。確定足だけで判定。チャートに出ている足（リアルタイムは120本）の中だけで探すので、元の3,000本より帯は少なめ。表示のみで、サインの判定・メールには使っていません。",
    kalmanNote:
