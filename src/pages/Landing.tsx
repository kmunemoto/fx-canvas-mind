...
const FAQS = [
  {
    q: "本当に勝てますか？",
    a: "勝率を保証するものではありません。AIによる分析は判断材料の1つとしてご活用ください。投資判断は最終的にユーザーご自身の責任で行ってください。",
  },
  {
    q: "解約はいつでもできますか？",
  ...
  {
    q: "対応している通貨ペアは？",
    a: "USD/JPY、EUR/USD、GBP/USD、EUR/JPY、GBP/JPY、AUD/USD、AUD/JPYに対応しています。",
  },
  {
    q: "スマホでも使えますか？",
    a: "はい、スマートフォンのブラウザから問題なくご利用いただけます。",
  },
];
    ...
    <div className="min-h-screen bg-[#0a0e17] text-foreground overflow-x-hidden">
      ...
      <section className="relative py-24 md:py-36 overflow-hidden">
        ...
        >
          <motion.h1 variants={fadeUp} className="text-4xl md:text-6xl font-bold leading-tight mb-6">
            AIがあなたの<br className="md:hidden" />
            <span className="text-[#00d4ff]">FXトレード</span>を分析
          </motion.h1>
          <motion.p variants={fadeUp} className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-10">
            ボタン1つで、<span className="font-mono-data text-foreground">11種</span>のテクニカル指標と最新の市場情報を統合分析
          </motion.p>
          <motion.div variants={fadeUp} className="flex flex-col items-center gap-4">
            ...
            >
              無料で始める <ArrowRight className="inline h-5 w-5 ml-1" />
            </button>
      ...
      <Section>
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-6" variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          {[
            ...
            <motion.div key={title} variants={fadeUp} className="glass rounded-2xl p-8 border border-white/5">
      ...
      <Section>
              ...
              <ul className="space-y-3 mb-6 flex-1">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm">
                    <Check className="h-4 w-4 text-[#00d4ff] mt-0.5 shrink-0" /> {f}
                  </li>
                ))}
              </ul>
            </motion.div>
        ...
        <div className="text-center mt-8">
          ...
          >
            料金詳細を見る <ChevronRight className="h-4 w-4" />
          </button>
        </div>
      </Section>
