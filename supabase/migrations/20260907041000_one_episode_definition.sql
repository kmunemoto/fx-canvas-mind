-- One definition of "an independent situation", and the screen now uses it.
--
-- Four implementations of clustering existed, and they differed in two ways at
-- once (measured 2026-09-07 against the working tree and the live database):
--
--   postmortem/prompt.ts        window anchored on the PREVIOUS plan, so each
--                               plan pushed the deadline out — a chain — plus
--                               a reopen escape that could never fire.
--   src/lib/outcomeStats.ts     window anchored on the CLUSTER'S START, no
--                               escape.
--   performance_stats (SQL)     the same fixed anchor, no escape. This one.
--
-- Under a steady cadence the two anchors diverge without bound: the chain
-- fuses a week of daily plans into one situation, the fixed anchor cuts it
-- into seven. The number that decided whether a rule SURVIVED a revision came
-- from the chain; the number printed under the win rate came from here. Same
-- name, different quantity, and no way to tell from either which was which.
--
-- The rule is now the union of the two halves that were each half right, and
-- it lives in one place: supabase/functions/_shared/episodes.ts. This function
-- is the SQL statement of that same rule.
--
-- Editing the earlier migration files would have changed nothing in the
-- database — 20260906060000 is the applied definition — so this replaces it.
--
-- Note on the four hours: it is a working aggregation threshold, not a claim
-- of statistical independence. It says the earlier trade was over before the
-- later one was written, not that the two are independent draws.
